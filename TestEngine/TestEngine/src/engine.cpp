...
class MyContactListener : public b2ContactListener
{
	void BeginContact(b2Contact* contact) {
		...
		if (userData)
		{
			...
			//std::cout << "COLLISION COLLISION: " << m->position.x << std::endl;
			//m->Destroy();
...
int32 velocityIterations = 8;//6;
int32 positionIterations = 3;//2;
...
namespace GameEngine {
	...
	void Engine::Update()
	{
		...
		while (isRunning) {
			...
			for (int i = 0; i < getLevel().background.size(); ++i)
			{
				...
				scrollRect.x =0;// mainLevel.background[i].scrollRect.w;
				scrollRect.y =0;// mainLevel.background[i].scrollRect.h;

				SDL_QueryTexture(background, NULL, NULL, &scrollRect.w, &scrollRect.h);

				//scrollPosition.x = scrollPosition2.x = 0;
				...
				scrollPosition.w = scrollPosition2.w = windowDisplay.windowWidth;//640;
				scrollPosition.h = scrollPosition2.h = windowDisplay.windowHeight;//480;
			...
			while (bodyList != nullptr)
			{
				...
				//std::cout << "OBJ: " << bodyList << "\n";
				//world.DestroyBody(bodyList);

			}

			/*
			for (int a = 0; a < body->GetWorld()->GetBodyCount(); a++) {
				//std::cout << "OBJ: " << &body->GetWorld()->GetBodyList() << "\n";
				//body->GetWorld()->GetBodyList()[a]
				//&body->GetWorld()->GetBodyList()[a]
				//body->GetWorld()->DestroyBody();
			}
			*/
			...
			for (int i = 0; i < getLevel().levelObjects.size(); ++i) {
				...
				if (spriteAnimation->tilemapPath != "") {
					//float bodyWidth = getLevel().levelObjects[i]->animation.frameWidth;
					//float bodyHeight = getLevel().levelObjects[i]->animation.frameHeight;
					
				}
				else
				{
					//dynamicBox.SetAsBox(32,32);
				...
				*/
				...
				//body->GetWorld()->DebugDraw();
				body->CreateFixture(&fixtureDef);
				//b2Fixture* fixture = body->CreateFixture(&fixtureDef);
				// 
				//body->SetLinearVelocity(b2Vec2(0, 0.0f));
					...
					else if (spriteAnimation->manual.empty() == false)
					{
						...
						if (spriteAnimation->frameTime > spriteAnimation->frameDuration)
						{
							...
							else
							{
								...
								{
									spriteAnimation->spriteIndex = spriteAnimation->manual.size() -1;
								}
								*/
								getLevel().levelObjects[i]->OnAnimationFinish();
							}
						...
						//std::cout << spriteAnimation->animationRect.x << "\n";
						//std::cout << "index: " << spriteAnimation->spriteIndex << "\n";
						...
						//SDL_RenderCopy(renderTarget, sprite, &spriteRect, &spritePos);
						//SDL_RenderCopyEx(renderTarget, sprite, &spriteRect, &spritePos, -90.0f, NULL, SDL_FLIP_NONE);
						SDL_DestroyTexture(sprite);
	...
	void Engine::Initialize(GameWindow windowSettings)
	{
		...
		//std::cout << "Index " << windowDisplay.window_height << " " << windowDisplay.window_name << std::endl;
...
/*
void Animation::Play(float delta)//change name from play to something else
{
	//std::cout << "Playing Animation" << animationRect.x << std::endl;
	//&UpdateDeltaTime();
	//(*UpdateDeltaTime)
	...
	*/
	...
	if (frameTime > 0.1f) {
		...
		if (animationRect.x >= textureWidth) {
			...
			if (animationRect.y >= textureHeight) {
				if (loop) {
					animationRect.y = 0;
				}
				else {
					animationRect.x = textureWidth - frameWidth;
					animationRect.y = textureHeight - frameHeight;
				}
			}
		}
	...
	*/
	/*
	SDL_Rect spriteRect;
	spriteRect.x = animationRect.x;
	spriteRect.y = animationRect.y;
	spriteRect.w = animationRect.w;
	spriteRect.h = animationRect.h;
	*/
	//SDL_RenderCopy(renderTarget, currentImage, animationRect, animPosition);
//}
